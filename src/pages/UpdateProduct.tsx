...
const UpdateProduct: FC<PropsType> = ({ pageName, collectionName, routeName, layout }) => {
  ...
  const [uploadedImages, setUploadedImages] = useState<string[]>(currentProduct.images); // Images téléchargées
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const fileUrl = await uploadFileService.uploadFile(file, "products", (rate) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await productsService.updateProduct(collectionName , product.id || '' , product);
        ...
        <div className="flex flex-col gap-9">
          ...
          <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <div className="flex flex-col gap-5.5 p-6.5">
              ...
              {
              layout!=='hotel' ?  <div>
                <label className="mb-3 block text-black dark:text-white">Quantité disponible</label>
                <input
                  type="number"
                  name="quantite"
            ...
            <div className="flex flex-col gap-5.5 p-6.5">
              ...
              >
                {loading ? "Modification  en cours..." : "Modifier le produit"}
                  ...
                  onClick={()=>{
                    navigate("/hotel/update-logement" , {state: product})
                  }}
                className="bg-blue-500 text-white px-4 py-2 rounded"
              >
                Continuer
