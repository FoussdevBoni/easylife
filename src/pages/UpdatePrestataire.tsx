...
const UpdatePrestataire: FC<PropsType> = ({ pageName, title, layout , routeName }) => {
    const location = useLocation();
  const merchant = location.state;
  const [autocomplete, setAutocomplete] = useState(null);

  // Vérifier si merchant est défini
  if (!merchant) {
    return <div>Aucune donnée de prestataire trouvée.</div>;
  }
  ...
  const [uploadedImageUrl, setUploadedImageUrl] = useState<string | null>(merchant.profile);
